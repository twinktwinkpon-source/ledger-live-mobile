...
export const accountsSelector = (state: { accounts: AccountsState }): Account[] => {
  ...
  if (envFlex) {
    ...
    flexCache = rawAccounts.map(sanitizeAccount);
    return flexCache;
  }
  flexCache = null;
  ...
  if (lastAccountsRef === accounts && lastSanitizedResult) {
    return lastSanitizedResult;
  }

  lastAccountsRef = accounts;
  lastSanitizedResult = accounts.map(sanitizeAccount);
  return lastSanitizedResult;
};
