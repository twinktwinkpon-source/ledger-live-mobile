...
async function buildAccountBridgeForFamily(family: string): Promise<ResolvedAccountBridge<any>> {
  ...
  if (isGenericCoinFrameworkFamily(family)) {
    ...
    if (!setup?.bridge) {
      // FLEX: return a mock bridge instead of crashing when the coin family is
      // not registered. In stock Ledger Live every family is registered so this
      // never runs; in FLEX builds some families may be absent and the mock
      // bridge lets the UI render without crashing.
      const mockP = loadMockBridgeForFamily(family);
      const mockBridge = mockP ? await mockP : undefined;
      if (mockBridge) {
        mockBridge.loadCoinConfig?.();
